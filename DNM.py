...
class DNM(BaseEstimator, TransformerMixin):
    ...
    def train(self, Train_data, Test_data):  # fit a dataset
        ...
        if self.verbose > 0:
            ...
            # init_test_rm, init_test_ma = self.evaluate(Test_data)
            print("Init:tr_rmse=[%.4f ,%.4f],tr_mae=[%.4f, %.4f] ,te_rmse=[%.4f ,%.4f],te_mae=[%.4f ,%.4f] ,te_nmae=[%.5f ,%.5f],te_mae_all=[%.3f] [%.1f s]" % (init_train_rm[0], init_train_rm[1], init_train_ma[0], \
            init_train_ma[1], init_test_rm[0] , init_test_rm[1], init_test_ma[0], init_test_ma[1], init_test_nma[0], init_test_nma[1], init_test_ma_all, time() - t2))
        ...
        for epoch in range(self.epoch):
            ...
            if self.verbose > 0 and epoch % self.verbose == 0:
                print("Epoch %d [%.1f s] tr_rmse=[%.4f ,%.4f],tr_mae=[%.4f ,%.4f] , te_rmse=[%.4f ,%.4f],te_mae=[%.4f ,%.4f],te_mae_all=[%.3f]  [%.1f s]"
                      % (epoch + 1, t2 - t1, train_result_rmse[0], train_result_rmse[1], train_result_mae[0], train_result_mae[1], test_result_rmse[0], test_result_rmse[1], test_result_mae[0], test_result_mae[1], test_result_mae_all, time() - t2))

    def evaluate_train(self, data):  # evaluate the results for an input set
        num_example1 = len(data['Y1'])
        num_example2 = len(data['Y2'])
        feed_dict = {self.train_user_features: data['X_U'], self.train_service_features: data['X_I'], self.train_labels1: [[x_ext] for x_ext in data['Y1']], self.train_labels2: [[x_ext] for x_ext in data['Y2']], self.dropout_keep: self.no_dropout, self.train_phase: False}              
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        print("DNM: dataset=%s, hidden_factor=%d, dropout_keep=%s, layers=%s, layers_rt=%s, layers_tp=%s, loss_type=%s,  #epoch=%d, batch=%d, lr=%.4f, lambda=%.4f, optimizer=%s, batch_norm=%d, activation=%s" 
              % (args.dataset, args.hidden_factor, args.keep_prob, args.layers, args.layers_rt, args.layers_tp, args.loss_type, args.epoch, args.batch_size, args.lr, args.lamda, args.optimizer, args.batch_norm, args.activation))
